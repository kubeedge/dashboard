...
const DeptForm: React.FC<DeptFormProps> = (props) => {
  ...
  const handleOk = async () => {
    ...
    const res = await editYaml(sessionStorage.getItem("nameSpace") ,values.metadata.name, getYAMLJS.parse(submitVal))
    console.log(res)
  ...
  return (
    <Modal width={840} title='YAML' visible={props.visible} destroyOnClose onOk={handleOk} onCancel={handleCancel}>
      <CodeMirror ref={myCodeMirrorRef} value={yamlStr} options={options} editorDidMount={handleBlur} onChange={handleChange}></CodeMirror>
    </Modal>
  );
};
