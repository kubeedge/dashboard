...
const handleAdd = async (fields: DeviceType) => {
  ...
  try {
    const resp = await addDevice(sessionStorage.getItem("nameSpace"), { ...fields });
    hide();
            ...
            getList(sessionStorage.getItem("nameSpace")).then((res) => {
              return {
                data: res.items.map(item => {
                  return { name: item.metadata.name, uid: item.metadata.uid, creationTimestamp: item.metadata.creationTimestamp, protocol: item.spec.protocol.customizedProtocol.protocolName, node: item.spec.nodeSelector.nodeSelectorTerms[0].matchExpressions[0].values[0] }
                }),
