...
export function getListByQuery(namespaces: string, name: string) {
  return request(`/apis/devices.kubeedge.io/v1alpha2/namespaces/${namespaces}/devicemodels/${name}`, {
   method: 'get'
  }) 
}
...
export function removeItem(namespaces: string, app: string) {
  return request(`/apis/devices.kubeedge.io/v1alpha2/namespaces/${namespaces}/devicemodels/${app}`, {
    method: 'delete',
  });
}
