...
const handleRemove = async (selectedRows: MenuType[]) => {
  ...
  try {
    const resp = await removeMenu(
      selectedRows.map((row) => row.menuId).join(",")
    );
    hide();
    if (resp.code === 200) {
...
const MenuTableList: React.FC = () => {
  ...
  const columns: ProColumns<MenuType>[] = [
    ...
    {
      title: (
        <FormattedMessage id="system.Menu.icon" defaultMessage="菜单图标" />
      ),
      dataIndex: "icon",
      valueType: "text",
      hideInSearch: true,
    ...
    {
      title: (
        <FormattedMessage id="system.Menu.perms" defaultMessage="权限标识" />
      ),
      dataIndex: "perms",
      valueType: "text",
    },
    {
      title: (
        <FormattedMessage id="system.Menu.path" defaultMessage="路由地址" />
      ),
      dataIndex: "path",
      valueType: "text",
      hideInSearch: true,
    ...
    {
      title: (
        <FormattedMessage id="system.Menu.status" defaultMessage="菜单状态" />
      ),
      dataIndex: "status",
      valueType: "select",
