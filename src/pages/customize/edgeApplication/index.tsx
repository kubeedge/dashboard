...
import { getList, removeItem, getYaml, addEdgeapplication } from "./service";
...
const handleAdd = async (fields: any) => {
  ...
  try {
    const resp = await addEdgeapplication(fields.metadata.namespace, {
      ...fields,
    });
    hide();
            ...
            getList(params.namespaceSetting).then((res) => {
              ...
              filteredRes.forEach(
                (item: { metadata: any; spec: any; status: any }) => {
                  edgeapplicationList.push({
                    name: item.metadata.name,
                    namespace: item.metadata.namespace,
                    uid: item.metadata.uid,
                    creationTimestamp: item.metadata.creationTimestamp,
                    targetNodeGroups:
                      item.spec?.workloadScope?.targetNodeGroups
                        ?.map((item) => item.name)
                        ?.join(",") || "-",
                  });
                }
              ...
              return {
                data: edgeapplicationList,
                total: edgeapplicationList.length,
                success: true,
              };
            })
          }
