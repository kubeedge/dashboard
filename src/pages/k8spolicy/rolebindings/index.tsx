...
const DeptTableList: React.FC = () => {
  ...
  const handleAdd = async (fields: DeptType) => {
    ...
    try {
      const resp = await addBinding(sessionStorage.getItem("nameSpace") ,{ ...fields });
      hide();
  ...
  const columns: ProColumns<listType>[] = [
    ...
    },
    {
      ...
      dataIndex: 'roleRef',
      valueType: 'text',
      render: (_, record) => <span>{record.apiGroup}<br />{record.kind}<br />{record.roleRefName}</span>
    },
    {
      title: '创建时间',
            ...
            getList(sessionStorage.getItem("nameSpace")).then((res) => {
              return {
                data: res.items.map(item => {
                  return { name: item.metadata.name, uid: item.metadata.uid, creationTimestamp: item.metadata.creationTimestamp, apiGroup: item.roleRef.apiGroup, kind: item.roleRef.kind, roleRefName: item.roleRef.name }
                }),
