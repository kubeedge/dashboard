...
import { getList, removeItem } from "@/api/edgeApplication";
...
const EdgeApplication = (props) => {
  ...
  const refetchDataList = async () => {
    const resp = await fetch(`/api/edgeapplicationList?namespaces=${sessionStorage.getItem('namespace')}`)
    const res = await resp.json()
    setAppList(handleListData(res.data))
  }
  ...
  const handleRemoveOne = async () => {
    ...
    const resp = await fetch(`/api/edgeapplicationRemove?namespaces=${sessionStorage.getItem('namespace')}&name=${deleteName}`);
    const res = await resp.json()
  ...
  const handleOk = async () => {
    ...
    const resp = await fetch(`/api/edgeapplicationAdd?namespaces=${sessionStorage.getItem('namespace')}`, {
      method: 'post',
  ...
  return (
    <>
      ...
      <DataCard title={'edgeApplication'} action={cardActions}>
        <DataTable titleList={tableTitleList} dataList={appList} />
      </DataCard>
